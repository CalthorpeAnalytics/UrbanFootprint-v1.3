...
class MultithreadProcess(threading.Thread):
    ...
    def __init__(self, queue, sql_job_string):
        threading.Thread.__init__(self)
        self.queue = queue
        self.sql_job_string = sql_job_string

    def run(self):
        while True:
            #grabs host from queue
            job = self.queue.get()
            Task = self.sql_job_string.format(job['start_id'], job['end_id'])
            execute_sql(Task)
...
def count_cores():
    ...
    for line in open('/proc/cpuinfo','r'):
        if 0 == string.find(line, "processor"):
            cores = cores + 1
