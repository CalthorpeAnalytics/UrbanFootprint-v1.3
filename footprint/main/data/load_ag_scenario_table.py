...
from footprint.client.configuration import resolve_client_module, resolve_fixture, InitFixture
from footprint.main.lib.functions import map_to_dict
from footprint.main.models import AgricultureFeature, Project, DbEntity
...
class AgScenarioLoader(SourceTableLoader):
    ...
    def open_region_table(self, region):
        path = os.path.join(settings.PROJECT_ROOT, 'footprint', 'client', 'configuration',
    ...
    def load_project_to_source_db(self, region):
        ...
        logger.debug("Running SQL: \n {0}".format(recreate_db_table_sql))
        ...
        try:
            ...
            logger.debug("Running INSERT SQL for {0} rows".format(len(source_table)))
