...
class BuiltFormImporter(BuiltFormDerivatives):
    ...
    def built_form_path(self, client):
        ...
        client_built_form_path = "{server_root}/footprint/client/configuration/{client}/built_form/import_csv".format(
            server_root=settings.SERVER_ROOT, client=client)
    ...
    def construct_primary_components(self, client='default'):
        ...
        for import_primary_component in self.load_crops_csv(client):
            agriculture_attribute_set = dict(
                name=import_primary_component.name,
                crop_yield=import_primary_component.crop_yield,
                unit_price=import_primary_component.unit_price,
                cost=import_primary_component.cost,
                water_consumption=import_primary_component.water_consumption,
                labor_input=import_primary_component.labor_input,
                truck_trips=import_primary_component.truck_trips
            )
