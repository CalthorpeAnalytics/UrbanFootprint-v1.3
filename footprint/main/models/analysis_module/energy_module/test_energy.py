...
def test_execute_energy(config_entity):

    logger.info("Executing Energy using {0}".format(config_entity))

    run_energy_calculations(config_entity)

    logger.info("Done executing Energy")
    logger.info("Executed Energy using {0}".format(config_entity))


def run_future_energy_calculations(config_entity):
    ...
    energy_class = config_entity.db_entity_feature_class(DbEntityKey.ENERGY)
...
def run_base_energy_calculations(config_entity):
...
def run_energy_calculations(config_entity):

    start_time = time.time()

    if isinstance(config_entity.subclassed_config_entity, FutureScenario):
        energy_output_list,options = run_future_energy_calculations(config_entity)
    else:
        energy_output_list,options = run_base_energy_calculations(config_entity)
    ...
    post_save_config_entity_analysis_module.send(sender=config_entity.__class__, config_entity=config_entity, module='energy')