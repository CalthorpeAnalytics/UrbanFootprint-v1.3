...
@app.task
def executeVmt(hash_id, user, config_entity):
    # Make sure all related models have been created before querying
    logger.info("Executing Vmt using {0}".format(config_entity))
    run_quarter_mile_buffers(config_entity)
    run_one_mile_buffers(config_entity)
    run_variable_buffers(config_entity)
    FeatureClassCreator(config_entity).ensure_dynamic_models()
    run_vmt_model(config_entity)

    logger.info("Done executing Vmt")
    logger.info("Executed Vmt using {0}".format(config_entity))
    send_message_to_client(user.id,
                           dict(event='VmtModelCompleted',
...
def run_vmt_model(config_entity):

    vmt_feature = {}

    one_mile_buffer_class = config_entity.feature_class_of_db_entity_key(Keys.DB_ABSTRACT_VMT_ONE_MILE_BUFFER_FEATURE)
    quarter_mile_buffer_class = config_entity.feature_class_of_db_entity_key(Keys.DB_ABSTRACT_VMT_QUARTER_MILE_BUFFER_FEATURE)
    variable_buffer_class = config_entity.feature_class_of_db_entity_key(Keys.DB_ABSTRACT_VMT_VARIABLE_BUFFER_FEATURE)
    vmt_result_class = config_entity.feature_class_of_db_entity_key(Keys.DB_ABSTRACT_VMT_FEATURE)
    ...
    if isinstance(config_entity.subclassed_config_entity, FutureScenario):
        ...
        demographic_class = config_entity.feature_class_of_db_entity_key(Keys.DB_ABSTRACT_END_STATE_DEMOGRAPHIC_FEATURE)
        features = scenario_class.objects.all()

        annotated_features = annotated_related_feature_class_pk_via_geographies(features, config_entity, [
        Keys.DB_ABSTRACT_VMT_ONE_MILE_BUFFER_FEATURE, Keys.DB_ABSTRACT_VMT_QUARTER_MILE_BUFFER_FEATURE,
        Keys.DB_ABSTRACT_VMT_VARIABLE_BUFFER_FEATURE, Keys.DB_ABSTRACT_END_STATE_DEMOGRAPHIC_FEATURE])
        future = True
        ...
        annotated_features = annotated_related_feature_class_pk_via_geographies(features, config_entity, [
        Keys.DB_ABSTRACT_VMT_ONE_MILE_BUFFER_FEATURE, Keys.DB_ABSTRACT_VMT_QUARTER_MILE_BUFFER_FEATURE,
        Keys.DB_ABSTRACT_VMT_VARIABLE_BUFFER_FEATURE, Keys.DB_ABSTRACT_BASE_DEMOGRAPHIC_FEATURE])
        future = False
    ...
    for feature in annotated_features:
        ...
        vmt_feature['emp_office'] = float(feature.emp_office_services)
