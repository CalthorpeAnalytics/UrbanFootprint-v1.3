...
def calculate_final_vmt_results(vmt_feature):
    ...
    if vmt_feature['acres_parcel_res'] + vmt_feature['acres_parcel_emp'] + vmt_feature['acres_parcel_mixed'] > 0.0:
        emp_den = (vmt_feature['emp_retail'] + vmt_feature['emp_restaccom'] + vmt_feature['emp_arts_entertainment'] +
                   vmt_feature[
                       'emp_office'] + vmt_feature['emp_public'] + vmt_feature['emp_industry']) / (
                      vmt_feature['acres_parcel_res'] + vmt_feature['acres_parcel_emp'] + vmt_feature[
                          'acres_parcel_mixed'])
    ...
    if vmt_feature['emp30m_transit'] is None:
        emp30m_den = 0.0
    else:
    ...
    if float(vmt_feature['pop']) > 0:
        vmt_feature['vmt_daily_per_capita'] = (
            float(((final_prod_hbw + final_prod_hbo + final_prod_nhb) * 2)) / float(vmt_feature['pop']))
    else:
        vmt_feature['vmt_daily_per_capita'] = 0

    if float(vmt_feature['hh']) > 0:
        vmt_feature['vmt_daily_per_hh'] = (
            float(((final_prod_hbw + final_prod_hbo + final_prod_nhb) * 2)) / float(vmt_feature['hh']))
    else:
        vmt_feature['vmt_daily_per_hh'] = 0

    vmt_feature['vmt_annual'] = final_vmt_total * 350.0
    vmt_feature['vmt_annual_w_trucks'] = final_vmt_total_trucks * 350.0

    if float(vmt_feature['pop']) > 0:
        vmt_feature['vmt_annual_per_capita'] = (
            float((((final_prod_hbw + final_prod_hbo + final_prod_nhb) * 2) * 347.0)) / float(vmt_feature['pop']))
    else:
        vmt_feature['vmt_annual_per_capita'] = 0

    if float(vmt_feature['hh']) > 0:
        vmt_feature['vmt_annual_per_hh'] = (
            float((((final_prod_hbw + final_prod_hbo + final_prod_nhb) * 2) * 347.0)) / float(vmt_feature['hh']))
    else:
