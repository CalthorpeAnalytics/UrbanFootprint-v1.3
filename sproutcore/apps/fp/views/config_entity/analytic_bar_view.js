...
Footprint.AnalyticBarView = SC.View.extend({
    ...
    content:function() {
        if (this.get('presentations') && (this.getPath('presentationsStatus') & SC.Record.READY)) {
            ...
            if (libraries.get('length') > 0) {
                ...
                logWarning("No default result library found for config_entity: %@".fmt(this.getPath('config_enity.name')))
            }
        }
    ...
    */
    ...
    labelView: SC.LabelView.extend({
        rawValue:null,
        rawValueBinding:SC.Binding.oneWay('.parentView.value'),
        value: function() {
            var value = this.get('rawValue');
