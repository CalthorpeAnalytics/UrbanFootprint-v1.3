...
Footprint.EditableBuildingUsePercentFieldsView = SC.View.extend(SC.Control, {
    ...
    content:null,
    allContent: null,
    // The BuildingUsePercent matching the BuildingUseDefinition if the former exists
    // The property watches teh buildingUsePercents list in case a child view instantiates on and adds it to the list
    // TODO insanely, listing to buildUsePercents.[] causes errors when we try to clone that childArray
//    buildingUsePercent: function() {
//        // Find a matching BuildingUsePercent if it exists
//        return (this.get('buildingUsePercents') || []).filter(function(buildingUsePercent) {
//            return buildingUsePercent.getPath('building_use_definition.id') == this.getPath('content.id');
//        }, this)[0];
//    }.property('content', 'buildingUsePercents', 'buildingUsePercentsStatus').cacheable(), //, 'buildingUsePercents.[]').cacheable(),
    ...
    sqftView: Footprint.EditableFieldView.extend({
        layout: {top: 175, left: 30, width: 120, height:30},
        contentBinding:SC.Binding.oneWay('.parentView.buildingUsePercent.square_feet_per_unit')
    }),
