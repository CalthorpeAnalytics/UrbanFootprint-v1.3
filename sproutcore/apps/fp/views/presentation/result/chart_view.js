    ...
    graphView: SC.View.extend({
        ...
        xScale: function() {
            return d3.scale.ordinal()
                ...
                .rangeRoundBands([0, this.get('width')], 20);
         ...
         */
        width: function() {
            return 284 - this.get('leftMarginWidth');
        }.property('leftMarginWidth').cacheable(),

        leftMarginWidth:function() {
            return 284 * this.get('margin').left;
        }.property().cacheable(),
         ...
         */
        height:function() {
            return 120 - this.get('topMarginHeight');
        }.property('topMarginHeight').cacheable(),
        /***
         * The calculated height of the bottom margin
         */
        // bottomMarginHeight:function() {
        //     return 145 * this.get('margin').bottom;
        // }.property().cacheable(),

        topMarginHeight:function() {
            return 120 * this.get('margin').top;
        }.property().cacheable(),
...
//            var data = d3.layout.stack()(raw_data);
        ...
        xAxis: function() {
            var xlabels = d3.scale.ordinal()
                ...
                .rangeRoundBands([0, this.get('width')]);
        ...
        yAxis: function() {
            var ylabels = d3.scale.linear()
                ...
                .domain([d3.min([this.get('yMin'), 0]), d3.max([0, this.get('yMax') * 1.15])])
                .range([this.get('height'), 15]);

            var ystackedlabels = d3.scale.linear()
                .domain([d3.min([this.get('yMin'), 0]), this.get('ySum') * 1.15])
                .range([this.get('height'), 15]);
            ...
            }.property('yMin', 'yMax', 'ySum', 'height', 'isStacked'),
