...
from common.utils.postgres_utils import build_postgres_conn_string, postgres_env_password_loaded
...
def manage_py(command):
    sudo(PYTHON_INTERPRETER + ' ' + PROJ_ROOT + '/manage.py ' + command, user=env.deploy_user)
...
@task
def fresh_install_urbanfootprint():
    """
    deletes all program files before attempting installation
    :return:
    """
...
def deploy_data():
    ...
    sudo('chown calthorpe /srv/calthorpe_media')
    sudo('chmod g+w /srv/calthorpe_media')
...
def install_mapnik():
    ...
    sudo("add-apt-repository ppa:mapnik/v2.1.0 --yes")
...
def patch_django_layermapping():
    ...
    sudo('ln -s {PROJ_ROOT}/footprint/utils/layermapping.py '
