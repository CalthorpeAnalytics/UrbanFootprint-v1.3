    ...
    if isinstance(dictitem, dict):
        for (tag, child) in dictitem.iteritems():
            if str(tag) == '_text':
                ...
                # ABL override. Handle _attrs keyword as attributes
                if (tag == '_attrs'):
                    for attr, value in child.iteritems():
...
def ConvertXmlToDict(xml, dictclass=XmlDictObject):
    """
    Converts an XML file, text or ElementTree Element to a dictionary

    """
    if type(xml) in (str, unicode, SafeUnicode):
