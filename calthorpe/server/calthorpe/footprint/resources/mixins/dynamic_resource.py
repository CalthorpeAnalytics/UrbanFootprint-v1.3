...
from footprint.lib.functions import remove_keys, merge
from footprint.models import ConfigEntity
from footprint.models.config.scenario import FutureScenario, BaseScenario
...
class DynamicResource(FootprintResource):
    ...
    def resolve_config_entity(self, params):
        ...
        if len(list(scenarios)) > 0:
            ...
            if len(list(scenarios)) > 0:
                ...
                return ConfigEntity.objects.filter(id=int(params['config_entity__id'])).select_subclasses()[0]
    ...
    def subclass_resource_if_needed(self, view, request):
        ...
        DynamicResourceClass = self.create_subclass(params, method=request.method)
        ...
        self.add_permissions_to_user(user, self.get_or_create_permissions_for_class(DynamicResourceClass, additional_classes_used, config_entity))
        ...
        return DynamicResourceClass().wrap_view(view)
    ...
    def get_or_create_permissions_for_class(self, DynamicResourceClass, additional_classes_used, config_entity):
        ...
        for clazz in [model_class] + additional_classes_used:
            return map(lambda action: Permission.objects.get_or_create(
                ...
                content_type_id=ContentType.objects.get(app_label="footprint", model=config_entity.__class__.__name__.lower()).id,
