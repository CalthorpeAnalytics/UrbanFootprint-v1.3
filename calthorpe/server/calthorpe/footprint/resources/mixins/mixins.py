...
from footprint.lib.functions import map_to_keyed_collections, flatten, map_to_dict, merge, unique, flat_map_values, get_first
from footprint.models import ResultLibrary, LayerLibrary
...
class BuiltFormSetsResourceMixin(ModelResource):
    ...
    built_form_sets = ToManyCustomAddField('footprint.resources.built_form_resources.BuiltFormSetResource', attribute=built_from_sets_query, add=add_built_form_sets, full=False, null=True)
...
class DbEntityResourceMixin(ModelResource):
    ...
    db_entity_interests = ToManyCustomAddField('footprint.resources.db_entity_resources.DbEntityInterestResource', attribute=db_entity_interests_query, add=add_db_entity_interests, full=False, null=True)
...
class PresentationResourceMixin(ModelResource):
    ...
    presentations_query = lambda bundle: bundle.obj.presentation_set.select_subclasses()
    presentations = ToManyFieldWithSubclasses('footprint.resources.presentation_resources.PresentationResource', attribute=presentations_query, full=False, null=True)
    ...
    def dehydrate_presentations(self, bundle):
        ...
        :return:
        """
        presentations = map_to_dict(lambda presentation: [str(presentation.id), presentation], bundle.obj.presentation_set.select_subclasses())
        return map_to_keyed_collections(lambda presentation_uri: self.map_uri_to_class_key(presentations, presentation_uri), bundle.data['presentations'])
...
class CategoryResourceMixin(ModelResource):
    categories = ToManyCustomAddField(CategoryResource, 'categories', full=True, add=add_categories)
