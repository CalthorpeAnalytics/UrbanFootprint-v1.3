...
from footprint.lib.functions import merge, map_to_dict
from footprint.utils.utils import table_name_only, string_not_empty, getSRIDForTable
from footprint.views import scenario_context
...
def tilestache_map(request, scenario_id, fullscreen):
    ...
    for l in config.layers:
        if isinstance(config.layers[l].provider, TileStache.Providers.Vector.Provider):
            ...
            #layer_urls.append(reverse('tilestache', current_app='footprint', args=[l, '{Z}', '{X}', '{Y}', 'geojson']).replace('%7B', '{').replace('%7D', '}')) # FAILS to resolve the tilestache view
