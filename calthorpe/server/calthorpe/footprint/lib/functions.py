...
def map_to_keyed_collections(lambda_call, list):
    ...
    :param lambda_call: Takes an item and returns the key to which it should map
    :param dictionary:
    :return:
    """
    result = {}
    for item in list:
        key = lambda_call(item)
...
def resolve_property(obj, property):
    ...
    :return:
    """
    value = obj.attr(property)
    return value() if isfunction else value


def map_property(dicts_or_objs, property):
    """
        Map the list of dicts or objects to property specified by the property string. attr() will be used to resolve object properties, and any property that represents a simple getter will be called as a no-arg function
