...
def resolve_client_module(module, module_fragment, schema=settings.CLIENT):
    """
        Resolve the client module under footprint.initialization.fixtures.client.[client], which client is the
    ...
    :return:
    """
    module_string = '%s.%s_%s' % (module, schema, module_fragment) if module else\
        '%s_%s' % (schema, module_fragment)

    return importlib.import_module("footprint.initialization.fixtures.client.%s.%s" % (schema, module_string))
...
def resolve_default_fixture(module, module_fragment, fixture_class, *args, **kwargs):
    ...
    :param module: module under footprint.initialization.fixtures.client.default
    ...
    client_module = importlib.import_module(
         "footprint.initialization.fixtures.client.default.%s" % default_module_string)
...
def resolve_fixture(module, module_fragment, fixture_class, schema=settings.CLIENT, *args, **kwargs):
    """
        ...
        "footprint.initialization.fixtures.client.[schema]" where schema is the matching ConfigEntity.schema().
...
def resolve_fixture_class(module, module_fragment, base_class, schema=settings.CLIENT):
    ...
    :param module: Module under footprint.fixtures.client.[client]
