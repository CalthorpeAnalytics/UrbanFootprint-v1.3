...
(function ($) {
	...
	var _container_popup = '<div class="ui-colorpicker ui-colorpicker-dialog ui-dialog ui-widget ui-widget-content ui-corner-all" style="display: none;"></div>',
		...
		_layoutTable = function(layout, callback) {
			...
			width = 0;
			height = 0;
            $.each(layout, function(index,l) {
			//for (index in layout) { // Removing this because it picks up non-indexed attributes
	...
	$.widget("vanderlee.colorpicker", {
		options: {
			...
			buttonImage:		'/img/colorpicker/ui-colorpicker.png',
		...
		_generate: function () {
			...
			if (!that.generated) {
                ...
                $.each(that.options.layout, function(index,l) {
		...
		_change: function (set /* = true */) {
			...
			if (!this.inline) {
				if (!this.color.set) {
                    ...
                    // Fix by Andy Likuski to format the input element value to the desired format
                    this.element.val(_formatColor(this.options.colorFormat, this.color));
                    // former code which ignores formatting choice
			...
			header: function (inst) {
				...
				_html = function () {
					...
					return '<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">'
						...
						+ '<span class="ui-dialog-titlebar-close ui-corner-all" role="button">'
						+ '<span class="ui-icon ui-icon-closethick">close</span></span></div>';
				};
