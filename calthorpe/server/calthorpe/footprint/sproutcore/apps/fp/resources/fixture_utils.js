...
Footprint.Scenario.FIXTURES_MULTIPLIER = 4;
Footprint.Scenario.FIXTURES_ID_MULTIPLIER = 10;
function multiplyScenarioId(baseId, index) {
    return baseId*Footprint.Scenario.FIXTURES_ID_MULTIPLIER + index;
}
...
function generateSortedLetters(name) {
    return name.toUpperCase().slice(0,Footprint.Scenario.FIXTURES_MULTIPLIER).split('').sort();
}
